...
<?php $component->withAttributes([]); ?>
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div class="flex items-center gap-4 mb-8">
            <a href="<?php echo e(route('orders.index')); ?>" class="text-indigo-600 hover:text-indigo-700 bg-indigo-50 p-2 rounded-full transition-colors flex items-center justify-center w-10 h-10">
            ...
            <div>
                ...
                <p class="text-slate-500 text-sm font-mono mt-1 font-bold"><?php echo e($order->qr_code); ?></p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-3xl p-8 shadow-sm border border-slate-100 relative overflow-hidden">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div class="space-y-6">
                            ...
                            <?php $__currentLoopData = $order->refunds; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $refund): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <div class="bg-white border border-slate-200 rounded-xl p-3 mb-2">
                                    <div class="flex justify-between items-center">
                                        <span class="font-bold text-slate-800">Rp <?php echo e(number_format($refund->amount, 0, ',', '.')); ?></span>
                                        <span class="text-xs px-2 py-1 rounded-full <?php echo e($refund->status === 'APPROVED' ? 'bg-green-100 text-green-700' : ($refund->status === 'REJECTED' ? 'bg-red-100 text-red-700' : 'bg-amber-100 text-amber-700')); ?>">
                                            <?php echo e($refund->status); ?>

                                        </span>
                                    </div>
                                    <p class="text-sm text-slate-600 mt-1"><?php echo e($refund->reason); ?></p>
                                    <?php if($refund->refunded_at): ?>
                                        <p class="text-xs text-slate-400 mt-1">Diproses <?php echo e($refund->refunded_at->format('d M Y, H:i')); ?></p>
                                    <?php endif; ?>
                                </div>
                    ...
                    <div>
                        ...
                        <p class="text-slate-800 font-bold text-lg"><?php echo e($order->created_at->format('d M Y, H:i')); ?></p>
                    </div>

                    <div>
                        <h2 class="text-sm font-bold text-slate-400 uppercase tracking-widest mb-1">Metode & Pengiriman</h2>
                        <p class="text-slate-800 font-medium">Pembayaran: <strong><?php echo e($order->payment_method === 'CASH' ? 'Tunai (Bayar di Tempat)' : 'Doku Non-Tunai'); ?></strong></p>
                        <p class="text-slate-800 font-medium mt-1">Pengiriman: <strong><?php echo e($order->delivery_method === 'AMBIL_LOKASI' ? 'Ambil di Tempat' : 'Kirim Alamat'); ?></strong></p>
                        <?php if($order->delivery_method === 'KIRIM_ALAMAT'): ?>
                            <p class="text-slate-600 mt-2 bg-slate-50 p-3 rounded-xl border border-slate-100 text-sm">
                                <strong>Alamat:</strong> <br><?php echo e($order->shipping_address); ?>
                ...
                <div class="flex flex-col items-center justify-center p-6 bg-slate-50 border-2 border-dashed border-slate-200 rounded-3xl text-center">
                    <?php if($order->status === 'PAID' || ($order->status === 'PENDING' && $order->payment_method === 'CASH')): ?>
                        ...
                        <div class="bg-white p-4 rounded-2xl shadow-sm inline-block">
                            <?php echo QrCode::size(180)->generate(route('seller.orders.show', ['order' => $order->id, 'scanned' => 'true'])); ?>

                        </div>
                        <p class="mt-3 text-indigo-600 font-bold font-mono tracking-widest text-sm"><?php echo e($order->qr_code); ?></p>

                        <?php if($order->seller_completion_notified_at && !$order->buyer_completed_at): ?>
                            ...
                            <form method="POST" action="<?php echo e(route('orders.complete', $order)); ?>" class="mt-4 w-full max-w-xs">
                                <?php echo csrf_field(); ?>
                        ...
                        <?php elseif(!$order->seller_completion_notified_at): ?>
                    ...
                    <?php elseif($order->status === 'PENDING' && $order->payment_method === 'NON_CASH'): ?>
                        ...
                        <?php if($order->doku_payment_url): ?>
                            <a href="<?php echo e($order->doku_payment_url); ?>" class="mt-6 w-full max-w-[200px] text-center bg-indigo-600 text-white font-bold py-3 px-6 rounded-xl hover:bg-indigo-700 shadow-md">
                                Bayar Sekarang
                            </a>
                        <?php endif; ?>
            ...
            <div class="border-t border-slate-100 mt-8 pt-8 pt-4">
                ...
                <div class="flex justify-between items-center bg-slate-50 p-6 rounded-2xl border border-slate-100">
                    ...
                    <span class="text-3xl font-black text-indigo-600">Rp <?php echo e(number_format($order->total_price, 0, ',', '.')); ?></span>
                </div>
            </div>
                        ...
                        <?php $__currentLoopData = $order->refunds; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $refund): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <div class="bg-white border border-slate-200 rounded-xl p-4">
                                <div class="flex justify-between items-center">
                                    <div>
                                        <span class="font-bold text-slate-800">Rp <?php echo e(number_format($refund->amount, 0, ',', '.')); ?></span>
                                        <?php if($refund->requested_by === 'BUYER'): ?>
                                    ...
                                    <span class="text-xs px-3 py-1 rounded-full font-bold
                                        <?php echo e($refund->status === 'APPROVED' ? 'bg-green-100 text-green-700' : ($refund->status === 'REJECTED' ? 'bg-red-100 text-red-700' : 'bg-amber-100 text-amber-700')); ?>">
                                        <?php echo e($refund->status === 'APPROVED' ? 'Disetujui' : ($refund->status === 'REJECTED' ? 'Ditolak' : 'Menunggu')); ?>

                                    </span>
                                </div>
                                <p class="text-sm text-slate-600 mt-2"><?php echo e($refund->reason); ?></p>
                                <?php if($refund->refunded_at): ?>
                                    <p class="text-xs text-slate-400 mt-1">Diproses <?php echo e($refund->refunded_at->format('d M Y, H:i')); ?></p>
                                <?php endif; ?>
                            </div>
            ...
            <?php if(in_array($order->status, ['PAID', 'COMPLETED']) && !$order->refunds->where('status', 'PENDING')->where('requested_by', 'BUYER')->count()): ?>
                <div class="border-t border-slate-100 mt-8 pt-6">
                    ...
                    <div class="bg-white rounded-2xl shadow-xl max-w-md w-full p-6 relative">
                        ...
                        <div class="text-center mb-5">
                            ...
                            <p class="text-sm text-slate-500 mt-1">
                                Pesanan <span class="font-mono font-bold"><?php echo e($order->qr_code); ?></span>
                            </p>
                        </div>
                        ...
                        <div class="bg-slate-50 rounded-xl p-4 mb-5">
                            <div class="flex justify-between items-center">
                                ...
                                <span class="text-lg font-bold text-slate-900">Rp <?php echo e(number_format($order->total_price, 0, ',', '.')); ?></span>
                            </div>
                        ...
                        <form method="POST" action="<?php echo e(route('orders.refund', $order)); ?>">
                            <?php echo csrf_field(); ?>
                            <div class="mb-5">
            ...
            <?php elseif($order->refunds->where('status', 'PENDING')->where('requested_by', 'BUYER')->count()): ?>
                <div class="border-t border-slate-100 mt-8 pt-6">
...
<?php endif; ?>
<?php /**PATH C:\Kopi_Ngaji_Project\KopiNgaji\resources\views\orders\show.blade.php ENDPATH**/ ?>
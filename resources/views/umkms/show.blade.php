<x-app-layout>
    ...
    <div class="relative bg-white pt-10 pb-6 shadow-sm border-b border-slate-200">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 flex flex-col md:flex-row items-center md:items-start gap-6 pt-6">
            ...
            <div class="flex-grow text-center md:text-left pt-2 pb-4">
                ...
                @if ($umkm->description)
                    <p class="text-slate-500 max-w-3xl leading-relaxed text-base">{{ $umkm->description }}</p>
                @else
                ...
                <div class="flex flex-wrap items-center justify-center md:justify-start gap-4 mt-6 text-sm font-medium text-slate-600">
                    <div class="flex items-center gap-2">
                        ...
                        <span id="product-total-count">{{ $products->total() }} Produk</span>
                    </div>
                    <div class="flex items-center gap-2">
                        <div class="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center text-slate-400">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div class="flex flex-col md:flex-row items-center justify-between mb-8 pb-6 border-b border-slate-200 gap-4">
            ...
            <form id="store-search-form" class="w-full md:w-auto relative group" onsubmit="return false;">
                <input type="text" name="productSearch" id="store-search-input" value="{{ $productSearch ?? '' }}" placeholder="Cari di toko ini..." 
                ...
                <button type="button" id="store-clear-btn" class="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 {{ empty($productSearch) ? 'hidden' : '' }}">
            ...
            @if ($products->isEmpty())
                <div class="bg-white rounded-3xl p-16 text-center border border-slate-100 shadow-sm max-w-2xl mx-auto">
                    ...
                    <p class="text-slate-500">
                        {{ $productSearch ? 'Tidak ada produk yang cocok dengan pencarian Anda.' : 'Toko ini belum menambahkan produk apapun untuk dijual.' }}
                    </p>
                </div>
            @else
                    ...
                    @foreach ($products as $product)
                        <div class="bg-white border text-left border-slate-100 rounded-2xl overflow-hidden shadow-sm hover:shadow-[0_8px_30px_rgb(0,0,0,0.06)] transition-all duration-300 group flex flex-col h-full relative hover:-translate-y-1">
                            ...
                            <div class="p-4 flex flex-col flex-grow">
                                ...
                                <a href="{{ route('products.show', $product) }}" class="font-bold text-[15px] leading-tight text-slate-800 hover:text-indigo-600 transition-colors mb-2 line-clamp-2">{{ $product->name }}</a>
                                <div class="mt-auto pt-3 flex items-end justify-between">
                                    ...
                                    <form action="{{ route('cart.store') }}" method="POST">
                                        ...
                                        <button type="submit" @disabled($product->stock < 1) class="w-8 h-8 rounded-full bg-slate-50 border border-slate-200 text-slate-500 flex items-center justify-center hover:bg-emerald-500 hover:border-emerald-500 hover:text-white transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const baseUrl = "{{ route('umkms.show', $umkm) }}";
        const cartUrl = "{{ route('cart.store') }}";
